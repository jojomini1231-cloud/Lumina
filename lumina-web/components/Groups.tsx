...
export const Groups: React.FC = () => {
  ...
  const selectAllFilteredModels = () => {
    ...
    activeProviders.forEach(provider => {
        const displayModels = provider.models.filter(m => m.toLowerCase().includes(modelFilter.toLowerCase()));
        displayModels.forEach(model => {
            const exists = newTargets.some(t => t.providerId === provider.id && t.model === model);
            if (!exists) {
                newTargets.push({
  ...
  return (
    <div className="space-y-6 relative flex flex-col h-full">
      ...
      >
        <div className="flex flex-col items-center">
            ...
            <p className="text-center text-gray-500 dark:text-gray-400 text-sm mb-6 leading-relaxed">
                {t('groups.deleteDesc', { name: '' })} <span className="font-bold text-gray-900 dark:text-white">{deleteModal.name}</span>?
            </p>
                ...
                {groups.length === 0 ? (
                    <div className="col-span-full">
                        <EmptyState 
                            ...
                            description={t('groups.createFirst')}
      ...
      <Modal
        ...
        title={editingGroup ? t('groups.modal.titleEdit') : t('groups.modal.titleAdd')}
        size="2xl"
      >
                ...
                <div className="flex flex-col bg-gray-50 dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 overflow-hidden shadow-sm">
                    <div className="p-3 border-b border-gray-200 dark:border-gray-800 bg-white dark:bg-[#1a1a1a] z-10 space-y-3">
                        <div className="flex justify-between items-center">
                            ...
                            {modelFilter && activeProviders.some(p => p.models.some(m => m.toLowerCase().includes(modelFilter.toLowerCase()))) && (
                                <button
                                    type="button"
                                    onClick={selectAllFilteredModels}
                                    className="text-xs font-bold text-indigo-600 hover:text-indigo-700 dark:text-indigo-400 dark:hover:text-indigo-300 px-2 py-1 rounded-md hover:bg-indigo-50 dark:hover:bg-indigo-900/20 transition-colors"
                                >
